...
int main(int, char**)
{
    ...
    WNDCLASSEXW wc = { sizeof(wc), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, L"Symmetric Group Explorer", nullptr };
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"Symmetric Group Explorer", WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, nullptr, nullptr, wc.hInstance, nullptr);
...
void SYMUI_order_window(bool& showWindow)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            ERROR_PROTECT
            SymUI::ProcessPermutationInput(i, inputBuffer, permutation);
            ERROR_CATCH
        }
    }
    ImGui::EndTable();
    ...
    if (ImGui::SliderInt(label.c_str(), &n, 1, 20))
    {
        ...
        if (prevN > n)
        {
            ...
            for (int i = 0; i < prevN - n; i++)
            {
                for (std::vector<int>* permutation : permVec)
                {
                    SymUI::ShrinkPermutationByOne(*permutation);
                }
            }
        }
